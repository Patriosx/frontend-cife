...
export default function Auth(props) {
	...
	//se pone nombre undefined ya que al hacer el login el nombre no se necesita
	...
	const onSubmit = async (data) => {
		...
		if (!tieneAcceso) {
			await axios
				.post(process.env.REACT_APP_BASE_URL + "/api/usuarios/alta", {
			...
			await axios
				.post(process.env.REACT_APP_BASE_URL + "/usuarios/login", {
					email: data.email,
	...
	return (
		<Card className="authentication">
			...
			<form className="card" onSubmit={handleSubmit(onSubmit)}>
				{!tieneAcceso && <input type="text" placeholder="Nombre de Usuario." {...register("nombre", { required: true, message: "Requerido" })} />}
				<input type="text" placeholder="Email" {...register("email", { required: true, pattern: /^\S+@\S+$/i })} />
				{errors.email && errors.email.type === "required" && <span>Se requiere e-mail válido</span>}
				{errors.email && errors.email.type === "pattern" && <span>Se requiere e-mail válido</span>}
				<input type="password" placeholder="Contraseña" {...register("password", { required: true, minLength: 6 })} />
				{errors.password && errors.password.type === "required" && <span>Se requiere contraseña</span>}
				{errors.password && errors.password.type === "minLength" && <span>Mínimo de 6 caracteres</span>}

				<button type="submit">{tieneAcceso ? "ACCESO" : "ALTA"} </button>
			</form>
			<button onClick={gestorDeCambioDeModo}>CAMBIAR A {tieneAcceso ? "ALTA" : "ACCESO"}</button>
		</Card>
	);
}
